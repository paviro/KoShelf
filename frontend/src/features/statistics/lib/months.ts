...
export function monthKeyAt(index: number): StatisticsMonthKey {
    const safeIndex = Math.min(Math.max(index, 0), STATISTICS_MONTH_KEYS.length - 1);
    return STATISTICS_MONTH_KEYS[safeIndex];
}

export function toShortMonthKey(monthKey: StatisticsMonthKey): `${StatisticsMonthKey}.short` {
