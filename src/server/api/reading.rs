...
pub(crate) async fn reading_summary(
    ...
    let data = statistics::summary(&reading_data, query);
...
pub(crate) async fn reading_metrics(
    ...
    let data = statistics::metrics(&reading_data, query);
    Ok(Json(ApiResponse::new(data)))
}
...
pub(crate) async fn reading_available_periods(
    ...
    let data = statistics::available_periods(&reading_data, query);
    Ok(Json(ApiResponse::new(data)))
}
...
pub(crate) async fn reading_calendar(
    ...
    let data = statistics::calendar(&reading_data, &state.library_repo, query).await;
    Ok(Json(ApiResponse::new(data)))
}
...
pub(crate) async fn reading_completions(
    ...
    let data = statistics::completions(&reading_data, &state.library_repo, query).await;
    Ok(Json(ApiResponse::new(data)))
}
