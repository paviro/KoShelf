...
fn test_filter_stats_per_book_per_day() {
    ...
    for i in 0..10 {
        page_stats.push(fixtures::page_stat(1, i, day1_ts + i * 60, 60));
    }

    // Day 1: Book 2 - 5 pages, 300s (fails both thresholds)
    for i in 0..5 {
        page_stats.push(fixtures::page_stat(2, i, day1_ts + i * 60, 60));
    }

    // Day 2: Book 1 - 2 pages, 3600s (passes time threshold)
    for i in 0..2 {
        page_stats.push(fixtures::page_stat(1, i + 10, day2_ts + i * 1800, 1800));
    }

    // Day 2: Book 2 - 15 pages, 200s (passes pages threshold)
    for i in 0..15 {
        page_stats.push(fixtures::page_stat(2, i + 5, day2_ts + i * 10, 13));
    }

    // Day 3: Book 1 - 3 pages, 100s (fails both)
    for i in 0..3 {
        page_stats.push(fixtures::page_stat(1, i + 12, day3_ts + i * 30, 33));
    }

    // Day 3: Book 2 - 4 pages, 500s (fails both)
    for i in 0..4 {
        page_stats.push(fixtures::page_stat(2, i + 20, day3_ts + i * 100, 125));
    }

    let books = vec![
        fixtures::stat_book(1, "abc", 0, ContentType::Book),
        fixtures::stat_book(2, "def", 0, ContentType::Book),
    ];
    let mut data = fixtures::statistics_data(books, page_stats);
