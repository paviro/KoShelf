use crate::models::{StatisticsData, PageStat, StatBook};
...
fn test_filter_stats_or_logic() {
    ...
    for ts in remaining_days {
        if ts >= 1672531200 && ts < 1672617600 { has_day1 = true; }
        if ts >= 1672617600 && ts < 1672704000 { has_day2 = true; }
        if ts >= 1672704000 { has_day3 = true; }
    }
