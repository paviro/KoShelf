                ...
                Ok(Event::Start(ref e)) => {
                    ...
                    match name {
                        ...
                        b"genre" if in_title_info => {
                            if let Ok(text) = reader.read_text(e.name()) {
                                let genre = unescape(&text).unwrap_or(Cow::Borrowed(&text)).into_owned();
                                if !genre.is_empty() {
                                    subjects.push(genre);
                                }
                            }
                        ...
                        b"first-name" if in_author => {
                            if let Ok(text) = reader.read_text(e.name()) {
                                first_name = unescape(&text).unwrap_or(Cow::Borrowed(&text)).into_owned();
                            }
                        }
                        b"middle-name" if in_author => {
                            if let Ok(text) = reader.read_text(e.name()) {
                                middle_name = unescape(&text).unwrap_or(Cow::Borrowed(&text)).into_owned();
                            }
                        }
                        b"last-name" if in_author => {
                            if let Ok(text) = reader.read_text(e.name()) {
                                last_name = unescape(&text).unwrap_or(Cow::Borrowed(&text)).into_owned();
                            }
                        }
                        ...
                        b"isbn" if in_publish_info => {
                            if let Ok(text) = reader.read_text(e.name()) {
                                let isbn = unescape(&text).unwrap_or(Cow::Borrowed(&text)).into_owned();
                                if !isbn.is_empty() {
